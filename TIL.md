...
XGBoost의 트리 부스터는 희소행렬에서 비어있는 원솔르 누락된 값으로 인식함. 따라서 원-핫 인코딩으로 만들어진 0도 누락된 값으로 처리되기 때문에 밀집 배열로 바꾸어 사용함

XGB는 예측 구간을 위한 도구를 제공하지 않지만 사이킷런의 `GradientBoostingRegressor` 는 loss 매개변수를 'quantile'로 지정하고 alpha 매개변수에 백분율로 구간을 지정하여 계산할 수 있음. 90% 예측구간을 얻기 위해 alpha를 각각 0.05, 0.95로 지정하고 훈련 세트에 대한 예측값을 만듦

=> quantile regression?

사이킷런 api와 호환되는 MAPIE 패키지는 jacknife+ 기법을 기반으로 예측 구간을 계산함

사이킷런은 코드가 완료되기 전 최상의 매개변수를 저장하기 위해 시간이 많이 소모되는 탐색을 중지할 수 있는 방법을 제공하지 않음.

`min_child_weight` 를 높이고, `subsample` 을 낮추면 일반화 성능을 높이는데 도움됨

 테스트 점수를 높이기 위해 하이퍼파라미터 튜닝 과정을 반복해서는 안됨. 하지만 테스트 점수를 확인한 후 약간의 조정은 가능함

모델을 서비스 장치로 배포하려면 훈련된 모델 객체를 `직렬화` 해야 함

사이킷런에서 만든 파이프라인 객체를 통째로 저장하려면 `pickle` 모듈을 사용해야 함

또한 사이킷런은 `joblib` 패키지를 사용한 직렬화를 추천함. joblib 패키지는 넘파이 배열이 포함된 경우 종종 더 좋은 효율로 압축함



# Ch11. Other GB library

LightGBM, CatBoost는 범주형 특성을 다루는 방식이 조금 다름

XGB는 기본적으로 깊이별 트리 성장 방식을 사용함
LightGBM은 리프별 트리 성장 방식을 사용함. 두 방식은 최종적으로 동일한 트리를 만들지만 깊이별 성장 방식은 동일한 깊이에 있는 노들를 차례대로 분할하고, 리프별 성장 방식은 분할을 통해 가장 높은 이익을 얻는 순서대로 노드를 분할함

LightGBM은 히스토그램 기반 그레이디언트 부스팅을 사용함. 특성 값을 구간으로 나누어 노드 분할을 하기 때문에 속도가 빠름. 기본적으로 256개의 구간을 사용. 구간 중 하나를 누락된 값을 위해 사용하므로 누락된 값을 따로 전처리할 필요가 없음. 메모리 사용량도 적음

XGB도 `tree_method` 매개변수를 hist(또는 gpu_hist)로 지정 시 히스토그램 기반 GB 사용 가능. 범주형 특성을 자동으로 처리하려면 gpu_hist에서만 가능

`get_dummies` 함수는 판다스 df를 간편하게 원핫인코딩으로 바꾸어주지만 사이킷런의 파이프라인과 통합되지 않으며, 모델을 실전에 배치했을때 새로운 데이터에 적용하기 어려움

`joblib` 를 사용하여 모델을 저장할 수 있음

LightGBM, XGB는 특성 중요도를 그래프로 출력할 수 있음 `plot_importance()`
또한 트리그래프도 시각화 할 수 있음 `plot_tree()`
